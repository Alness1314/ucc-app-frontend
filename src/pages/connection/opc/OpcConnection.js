...
import { getColumns } from "./TableDataOpc"
import PropTypes from 'prop-types';

export default function OpcConnection({ darkMode }) {
  OpcConnection.propTypes = {
  ...
  const fetchData = async () => {
    apiService.get(`${process.env.REACT_APP_API_URL}${process.env.REACT_APP_API_PREFIX}/opc-conexion`, null, true)
      .then(response => {
  ...
  const handleDetails = (id) => {
    navigate(`/opc-server/details/${id}`);
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      apiService.delete(`${process.env.REACT_APP_API_URL}${process.env.REACT_APP_API_PREFIX}/opc-conexion/${id}`, true)
        .then(response => {
          if (response.status >= 200 && response.status < 300) {
            SweetAlert2({
              title: "Eliminado",
